<?php
    ...
    $query_msg = "SELECT * FROM message order by date DESC";
            ...
            <div class="position-absolute z-2 w-100 main">
                <div class="w-100 vh-auto row px-5 py-2 my-2">
                    <div class="bg-white col-12 p-2 mx-2 rounded-4 row justify-content-center shadow mb-4 text-center">
                        ...
                        <a href="edit_contact_us.php" class="col-1 column-center btn btn-white mx-1 hov-set" style="opacity: .5; scale: 1.15;">
                    ...
                    <form method="POST" action="set_contact_process.php" enctype="multipart/form-data" class="bg-white col-12 p-4 mx-2 rounded-4 row shadow mb-4">
                        <div class="w-100">
                        ...
                        <div class="col-6 px-2 my-2">
                            <label class="fw-light" for="titleFeatureContact"><small>Title</small></label>
                            <input type="text" name="titleFeatureContact" id="titleFeatureContact" class="form-control" value="<?= $result_contact['title'] ?>" required>
                        </div>
                        <div class="col-6 px-2 my-2">
                            <label class="fw-light" for="featureImage"><small>Feature Image</small></label>
                            <input type="file" name="featureImage" id="featureImage" class="form-control" onchange="readURL(event)" accept="image/*" value="" required>
                        </div>
                        <div class="col-12 px-2 my-2">
                            <label class="fw-light" for="locCompany">
                                <small>Company Location</small>
                                <sub>(ex. PPTQ Ibnu Abbas 2 Klaten)</sub>
                            </label>
                            <input type="text" name="locCompany" id="locCompany" class="form-control" value="<?= $result_contact['locCompany'] ?>" required>
                        </div>
                        ...
                        <!-- <div id="outputImg" class="w-100 rounded-5 mt-2"
                        style="
                        background-image: 
                        url('./assets/library_image/<?= $result_contact['featureImg'] ?>');
                        background-position: center; background-size: contain;
                        background-repeat: no-repeat;
                        height: 20rem;
                        "></div> -->
                    </form>
                    <div class="bg-white col-12 p-4 mx-2 rounded-4 row shadow mb-4">
                        <?php
                            $count_msg = mysqli_num_rows($result_msg);
                        ?>
                        <h3 class="text-dark fw-bold lh-1 position-relative">
                            Messages<span class="fs-7 position-absolute bg-danger rounded-pill text-white px-2 py-1"
                            style="left:9.7rem; top:-5px; user-select:none;"><?= $count_msg ?></span>
                        </h3>
                        <div class="mt-2 w-100 row">
                            <?php
                            while($data_msg = mysqli_fetch_assoc($result_msg)) {
                            ?>
                            <div class="p-1 px-2 col-4">
                                <div class="content-question bg-dark text-light shadow rounded-2 w-100" data-aos="fade-up" data-aos-duration="800">
                                    <div class="question d-flex justify-content-between px-3 py-2">
                                        <div>
                                            <span class="fw-semibold text-capitalize word-break"><?= $data_msg['name'] ?></span> | 
                                            <a href="mailto:jeki@mail.com?subject=NJOTRAVEL%20berterima%20kasih&body=Terima kasih%20sudah%20memberi%20pesan%20dan%20saran%20kepada%20kami,%20NJOTRAVEL" class="fs-7 fw-light text-decoration-none text-light toEmail" target="_blank"><?= $data_msg['email'] ?>
                                                <div class="popUpEmail py-1 lh-1 px-3 rounded-pill bg-primary text-white fw-light">
                                                    Balas?
                                                    <div class="item-pop-up-error position-absolute"></div>
                                                </div>
                                            </a>
                                        </div>
                                        <a href="delete_msg_process.php?id=<?= $data_msg['id'] ?>" class="btn btn-danger rounded-circle" style="scale: 0.75;">✖</a>
                                    </div>
                                    <div class="content-answer text-dark">
                                        <div class="answer">
                                            <?= $data_msg['message'] ?>
                                        </div>
                                    </div>
    ...
    <!-- <script>
        var readURL = function(event) {
            ...
            reader.onload = function() {
                ...
                outputImg.style.backgroundImage = dataURL;
            };
