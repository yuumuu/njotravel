...
if(isset($_POST['submitSettings'])) {
    $titleHome = strtolower($_POST['titleFeatureHome']);
    $featureDesc = $_POST['descMain'];
    ...
    $extension_photo_feature = strtolower(end($extension_raw));
    ...
    $extension_photo_banner = strtolower(end($extension_raw));
    ...
    if(isset($_FILES['featureImage']) && isset($_FILES['bannerImage'])) {
        if($featureImage_size < 5000000 && $bannerImage_size < 5000000) {
                move_uploaded_file($photoSponsor_tmp, $target_files . $featureName_new);
                move_uploaded_file($photoSponsor_tmp, $target_files . $bannerName_new);

                $query = "INSERT INTO set_home VALUES (NULL, '$titleHome', '$featureName_new', '$featureDesc', '$bannerName_new', '$bannerDesc', NOW());";
