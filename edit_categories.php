<?php
    ...
    $id_cat = $_GET['id'];
    $sql_cat = mysqli_query($conn, "SELECT * FROM daftar_kategori where id = $id_cat");
    $rslt_cat = mysqli_fetch_assoc($sql_cat);
?>
<!DOCTYPE html>
<html lang="en">
<head>
                ...
                <div class="w-100 vh-auto row px-5 py-2 my-2">
                    <div class="bg-white col p-4 mx-2 rounded-4 shadow">
                        <div class="d-flex justify-content-between px-4">
                            <h3 class="text-dark fw-bold">Categories</h3>
                            <a href="<?= $url; ?>" class="btn btn-danger bg-gradient shadow p-1 pt-2 px-3 rounded-pill">Back</a>
                        </div>
                        <div class="row ps-3 w-100">
                            <div class="col-12 py-3">
                                <h5 class="text-start fw-bold px-3 py-1 rounded-3 bg-danger bg-gradient text-white">Edit Category</h5>
                                <form action="edit_categories_process.php" method="post">
                                    <input type="hidden" name="id" id="id" value="<?= $rslt_cat['id'] ?>">
                                    <input type="text" name="nameCategory" id="nameCategory" placeholder="name" class="form-control py-1" autocomplete="off" value="<?= $rslt_cat['name'] ?>" required>
                                        ...
                                        <?php
                                            ...
                                            while($row_kat = mysqli_fetch_assoc($hasil_kat)) {
                                                ...
                                                if($row_kat['parent'] == NULL) {
                                                    $html = "<option value='" . $row_kat['name'] .
                                                    "'>" . ucwords($row_kat['name']) . "</option>";
                                                }
                                    ...
                                    <textarea name="descCategory" id="descCategory" placeholder="description" maxlength="100" rows="10" cols="30" class="form-control py-1" required><?= $rslt_cat['description'] ?></textarea>
                                    <input type="submit" name="submitCategory" id="submitCategory" value="Save Change" class="btn btn-dark bg-gradient border-0 py-2 px-3 mt-3 submitCategory">
                                </form>
                            </div>
                        </div>
                    </div>
