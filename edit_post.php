<?php
    ...
    $query_login = mysqli_query($conn, "SELECT * FROM daftar_user where id = $id_author");
...
<head>
    ...
    />
    <script src="https://cdn.tiny.cloud/1/5ch4ag3vus9c6mztzuxnrzdnjht1cujv9xp6ax7pofocpkcl/tinymce/6/tinymce.min.js" referrerpolicy="origin"></script>
    <style>
        ...
        <div class="col-10 vh-100 position-relative bg-light overflow-y-auto z-1 d-flex flex-column justify-content-start align-items-start p-0">
            ...
            <div class="position-absolute z-2 w-100 main">
                <div class="w-100 vh-auto row px-5 py-2 my-2">
                    <?php $titlePost = str_replace("-", " ", $artikel['titlePost']); ?>
                    <form method="POST" action="edit_post_process.php" enctype="multipart/form-data" class="bg-white col p-4 mx-2 rounded-4 row shadow w-100">
                        <div class="col-8 p-2">
                            <input value="<?= $artikel['id']; ?>" type="hidden" name="id" id="id">
                            <input value="<?= $titlePost; ?>" type="text" name="titlePost" id="titlePost" placeholder="Add Title" class="form-control shadow-sm fs-4 fw-normal p-2 px-3 mb-3">
                            <textarea name="editorPost" id="editorPost" class="shadow-sm">
                                <?= $artikel['editorPost']; ?>
                            </textarea>
                        </div>
                        <div class="col-4 p-2">
                            <div class="w-100 py-2 px-3 my-3 shadow-sm rounded-2 border">
                                Categories
                                    ...
                                    <?php
                                        ...
                                        while($row_kat = mysqli_fetch_assoc($hasil_kat)) {
                                            ...
                                            if($row_kat['parent'] == NULL) {
                                                $html = "<option value='" . $row_kat['name'] . "' " . (($artikel['categoryPost']) == $row_kat['name'] ? "selected" : "") . ">" . ucwords($row_kat['name']) . "</option>";
                                            } else {
                                                $html = "<option value='" . $row_kat['name'] . "' " . (($artikel['categoryPost']) == $row_kat['name'] ? "selected" : "") . ">&mdash; " . ucwords($row_kat['name']) . "</option>";
                                            }
                                            echo $html;
                                        }
                                    ?>
                                ...
                                <div class="w-100 py-2 px-3 my-3 shadow-sm rounded-2 border">
                                    Featured Image
                                    <input type="file" name="featureImage" id="featureImage" class="btn btn-light w-100 py-2">
                                </div>
                                <div class="w-100 my-1 rounded-2">
                                    <input type="submit" name="publishPost" value="Publish" class="btn btn-dark bg-gradient border-0 p-2 fw-semibold px-5 rounded-3 shadow-sm">
                                </div>
                            </div>
                    </form>
                </div>
