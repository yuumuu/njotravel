...
<body class="bg-primary overflow-hidden bg-orange position-relative">
            ...
            <div class="d-flex flex-column justify-content-start align-items-start w-100 position-relative overflow-y-auto sidebar">
                ...
                <div class="nav-side text-white w-100 pb-2">
                    ...
                    <div onclick="dropdownActive(this)" class="nav-link position-relative fs-5 py-2 bg-orange ps-3 fw-bold text-dark dropdown dropdown-active">
                        ...
                        <a href="media_library.php" class="nav-link fw-semibold"><li class="lh-1 py-2 fs-6">Library</li></a>
        ...
        <div class="col-10 vh-100 position-relative bg-light overflow-y-auto z-1 d-flex flex-column justify-content-start align-items-start p-0">
            ...
            <div class="position-absolute z-2 w-100 main">
                <div class="w-100 vh-auto row px-5 py-2 my-2">
                    <div class="bg-white col-12 p-4 mx-2 rounded-4 row shadow mb-5">
                        <div class="w-100 d-flex justify-content-between align-items-center">
                            <h3 class="text-dark fw-bold">Library</h3>
                            <a href="media_new.php" class="btn btn-dark fw-semibold py-2 px-3 rounded-4">Add New &plus;</a>
                        </div>
                        <div class="w-100 d-flex justify-content-start align-items-start">
                        </div>
                        <div class="w-100 p-3 row">
                            <?php
                                $query_library = "select * from daftar_gambar";
                                $hasil_library = mysqli_query($conn, $query_library);
                                
                                while($data_gambar = mysqli_fetch_assoc($hasil_library)) {
                            ?>
                            <div class="col-2 p-2">
                                <div class="img-sh bg-light overflow-hidden position-relative shadow border"
                                 style="background-image: url('./assets/library_image/<?= $data_gambar['fileName'] ?>');">
                                    <div class="w-100 position-absolute bg-img-library d-flex justify-content-center align-items-end pb-2">
                                        <button id="previewOpen" src="<?= $data_gambar['fileName'] ?>" onclick="previewOpen()" class="btn btn-warning rounded-2 shadow-sm text-white btn-media m-1">
                                            &#8505;
                                        </button>
                                        <form action="delete_media_process.php" method="post" id="deleteForm">
                                            <input type="hidden" value="<?= $data_gambar['id'] ?>" name="id" id="id">
                                            <input type="hidden" value="<?= $data_gambar['fileName'] ?>" name="fileName" id="fileName">
                                            <button type="submit" class="btn btn-danger rounded-2 shadow-sm text-white btn-media m-1">&#10006;</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
    ...
    <div class="preview_image">
        <div class="pop-up-preview rounded-4 overflow-hidden position-relative w-100 d-flex justify-content-center align-items-start">
            <button id="previewClose" onclick="previewClose(this)" class="close-btn btn btn-light fs-5 rounded-circle position-absolute fw-bold">
                &#10006;
            </button>
            <div class="bg-preview-img h-end p-5 border rounded-4 w-100"></div>
        </div>
    </div>

    <script>
        document.querySelectorAll("#previewOpen").forEach(e => {
            e.onclick = () => {
                var popUp = document.querySelector(".preview_image");
                var imgPopUp = document.querySelector(".preview_image .bg-preview-img");
                var fileName = e.getAttribute("src");

                popUp.style.display = "block";
                imgPopUp.style.backgroundImage = "url('./assets/library_image/" + fileName + "')";
            }
        });
        
        function previewClose(close) {
            var popUp = document.querySelector(".preview_image");
