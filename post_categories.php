...
<head>
    ...
    <title>Category - Dashboard | Njotravel</title>
</head>
            ...
            <div class="d-flex flex-column justify-content-start align-items-start w-100 position-relative overflow-y-auto sidebar">
                ...
                <div class="nav-side text-white w-100 pb-2">
                    ...
                    <div onclick="dropdownActive(this)" class="nav-link position-relative fs-5 py-2 bg-orange ps-3 fw-bold text-dark dropdown dropdown-active">
                        ...
                        <a href="post_categories.php" class="nav-link fw-semibold"><li class="lh-1 py-2 fs-6">Categories</li></a>
                    </div>
        ...
        <div class="col-10 vh-100 position-relative bg-light overflow-y-auto z-1 d-flex flex-column justify-content-start align-items-start p-0">
            ...
            <div class="position-absolute z-2 w-100 main">
                <div class="w-100 vh-auto row px-5 py-2 my-2">
                    <div class="bg-white col p-4 mx-2 rounded-4 shadow">
                        <h3 class="text-dark fw-bold ms-2">Categories</h3>
                        <div class="row ps-3 w-100">
                            <div class="col-3 shadow-sm p-3">
                                <h5 class="text-start fw-bold px-3 py-1 rounded-3 bg-danger bg-gradient text-white">Add New &plus;</h5>
                                <form action="add_category_process.php" method="post">
                                    <input type="text" name="nameCategory" id="nameCategory" placeholder="name" class="form-control py-1" autocomplete="off" required>
                                    <select name="parentCategory" id="parentCategory" class="form-control py-1 my-3">
                                        <option value="">-- parent category --</option>
                                        <?php
                                            include "koneksi.php"; global $conn;

                                            $query_kat = "select * from daftar_kategori where parent = ''";
                                    ...
                                    <textarea name="descCategory" id="descCategory" placeholder="description" maxlength="100" rows="10" cols="30" class="form-control py-1" required></textarea>
                                    <input type="submit" name="submitCategory" id="submitCategory" value="Add New Category" class="btn btn-dark bg-gradient border-0 py-2 px-3 mt-3 submitCategory">
                                </form>
                            </div>
                            <div class="col-9 shadow-sm p-3">
                                <h5 class="text-start fw-bold px-3 py-1 rounded-3 bg-danger bg-gradient text-white">All Categories</h5>
                                <table class="w-100 mt-3">
                                    <thead class="w-100">
                                        <tr class="w-100 row fs-5 text-center py-2 bg-white rounded-5">
                                            <th class="col-1 fw-semibold">No</th>
                                            <th class="col-2 fw-semibold">Name</th>
                                            <th class="col-4 fw-semibold">Description</th>
                                            <th class="col-3 fw-semibold">Actions</th>
                                            <th class="col-2 fw-semibold">Count</th>
                                        </tr>
                                    </thead>
                                    <tbody class="w-100">
                                        <?php
                                            ...
                                            $no = 1;
                                            while($data_cat = mysqli_fetch_assoc($hasil)) {
                                        ...
                                        ?>
                                        <tr class="row_kat w-100 row fs-6 text-center py-3">
                                            <td class="col-1 px-2 fw-normal"><?= $no++; ?></td>
                                            <td class="col-2 px-2 fw-normal"><?= ucwords($data_cat_name); ?></td>
                                            <td class="col-4 px-2 fw-normal text-break"><?= $data_cat['description']; ?></td>
                                            <td class="col-3 px-2 fw-normal d-flex justify-content-center align-items-start">
                                                <a href="edit_categories.php?id=<?= $data_cat['id'] ?>" class="btn btn-dark py-1 px-2 rounded-5 shadow me-1">
                                                    &#9998;
                                                </a>
                                                <form action="delete_category_process.php" method="POST">
                                                    <input type="hidden" value="<?= $data_cat['id'] ?>" name="id" id="id">
                                                    <button title="delete" class="btn btn-danger py-1 rounded-5 shadow">
                                                        &#10006;
                                                    </button>
                                                </form>
                                            </td>
                                            <td class="col-2 px-2 fw-normal">
                                                <?php
                                                    
                                                        $queryCount = "select * from daftar_artikel where categoryPost = '" . $data_cat['name'] . "';";
                                                        $hasilCount = mysqli_query($conn, $queryCount);
                                                        $dataCount = mysqli_num_rows($hasilCount);
                                                        echo $dataCount;
