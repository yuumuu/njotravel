<?php
    ...
    $query_user = "select * from daftar_user";
...
<head>
    ...
    <title>Dashboard | Njotravel</title>
</head>
<body class="bg-primary overflow-hidden bg-orange">
        ...
        <div class="col-2 vh-100 bg-dark overflow-hidden shadow z-3 d-flex flex-column justify-content-between align-items-start p-0 ps-3">
            <div class="d-flex flex-column justify-content-start align-items-start w-100 position-relative overflow-y-auto sidebar">
                ...
                <div class="nav-side text-white w-100 pb-2">
                    <a href="dashboard.php" class="nav-link w-100 fs-5 py-2 ps-3 fw-bold text-dark bg-orange">
                        Dashboard
                    </a>
        ...
        <div class="col-10 vh-100 position-relative bg-light overflow-y-auto z-1 d-flex flex-column justify-content-start align-items-start p-0">
            <div class="weltext position-absolute top-0 d-flex justify-content-between align-items-start px-5 py-3 pb-5 w-100 bg-orange">
                <h4 class="text-dark ms-3">
                    ...
                    <br>
                    <span class="fs-6 fw-semibold bg-white rounded-3 px-5">Dashboard</span>
                </h4>
            ...
            <div class="position-absolute z-2 w-100 main">
                <div class="w-100 row px-5 ">
                    <?php
                        $auth_user = $query_user . " where role = 'Author'";

                        $jumlah_user = mysqli_query($conn, $query_user);
                        $jumlah_auth_user = mysqli_query($conn, $auth_user);

                        $all_user = mysqli_num_rows($jumlah_user);
                        $all_auth_user = mysqli_num_rows($jumlah_auth_user);
                    ?>
                    <div class="col-4 p-2">
                        <div class="card-info shadow bg-white py-4 rounded-4 w-100 d-flex flex-column justify-content-center align-items-center">
                            <h5 class="fw-light lh-1">Total Users</h5>
                            <p class="h1 fw-bold lh-1 m-0"><?= $all_user; ?></p>
                        </div>
                    </div>
                    <div class="col-4 p-2">
                        <div class="card-info shadow bg-white py-4 rounded-4 w-100 d-flex flex-column justify-content-center align-items-center">
                            <h5 class="fw-light lh-1">Total Authors</h5>
                            <p class="h1 fw-bold lh-1 m-0"><?= $all_auth_user; ?></p>
                        </div>
                    </div>
                    <?php
                        $query_post = "select * from daftar_artikel";

                        $jumlah_post = mysqli_query($conn, $query_post);

                        $all_post = mysqli_num_rows($jumlah_post);
                    ?>
                    <div class="col-4 p-2">
                        <div class="card-info shadow bg-white py-4 rounded-4 w-100 d-flex flex-column justify-content-center align-items-center">
                            <h5 class="fw-light lh-1">Total Posts</h5>
                            <p class="h1 fw-bold lh-1 m-0"><?= $all_post; ?></p>
                        </div>
                    </div>
                </div>
                <div class="w-100 vh-auto row px-5 py-2 my-2">
                    <div class="bg-white col p-4 mx-2 rounded-4 row shadow mb-5">
                        <div class="col-8 p-3">
                            <div class="rounded-2 p-2 w-100 vh-auto p-4 pt-2">
                                <h3 class="lh-1 fw-semibold m-0">Traffic Post</h3>
                                <h6 class="lh-1 fw-light m-0">all post in one years</h6>
                                <canvas id="chartTraffic" class="w-100" width="20" height="13"></canvas>
                            </div>
                        </div>
                        <div class="col-4 p-3">
                            <div class="rounded-2 p-2 w-100 vh-auto">
                                <h3 class="lh-1 fw-semibold m-0">Users Chart</h3>
                                <h6 class="lh-1 fw-light mb-4">all user now</h6>
                                <canvas id="chartCircle" class="w-100 mb-5" width="20" height="13"></canvas>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <?php
        $auth_user = $query_user . " where role = 'Author'";
        $admin_user = $query_user . " where role = 'Admin'";

        $jumlah_auth_user = mysqli_query($conn, $auth_user);
        $jumlah_admin_user = mysqli_query($conn, $admin_user);

        $all_auth_user = mysqli_num_rows($jumlah_auth_user);
        $all_admin_user = mysqli_num_rows($jumlah_admin_user);
    ...
    <script>
        ...
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                label: 'Post in One Month',
                data: [
                <?php
                    $d = 0;
                    foreach($array_bulan as $arrs) {
                        $d++;

                        if($d < 10) {
                            $querys = "select * from daftar_artikel where datePost like '%-0" . $d . "-%'";
                        } else {
                            $querys = "select * from daftar_artikel where datePost like '%-" . $d . "-%'";
                        }

                        $hasil = mysqli_query($conn, $querys);
        ...
        // chart.js side
        var chartCircle = document.getElementById('chartCircle').getContext("2d");
