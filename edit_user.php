...
<head>
    ...
    <title>Edit Users - Dashboard | Njotravel</title>
</head>
        ...
        <div class="col-10 vh-100 position-relative bg-light overflow-y-auto z-1 d-flex flex-column justify-content-start align-items-start p-0">
            <div class="weltext position-absolute top-0 d-flex justify-content-between align-items-start px-5 py-3 pb-5 w-100 bg-orange">
                <h4 class="text-dark ms-3">
                    ...
                    <span class="fs-6 fw-semibold bg-white rounded-3 px-5">
                        <span class="ms-1 bg-danger bg-gradient py-0 px-3 rounded-5 fs-6 text-center fw-normal text-white">Edit</span>
                        User
                    </span>
                </h4>
            ...
            <div class="position-absolute z-2 w-100 main">
                <div class="w-100 vh-auto row px-5 py-2 my-2">
                    <div class="bg-white col p-4 mx-2 rounded-4 shadow w-100">
                        <div class="d-flex justify-content-between align-items-start w-100">
                            <div>
                                <h3 class="text-dark fw-bold lh-1">Edit User</h3>
                                <h6 class="text-dark fw-light lh-1 mb-4">Edit some data below for newest data user account</h6>
                            </div>
                            <a href="user_all.php" class="btn btn-danger bg-gradient shadow p-1 px-3 rounded-5">Back</a>
                        </div>
                        <form action="edit_user_process.php" method="POST" enctype="multipart/form-data" class="row">
                            <input type="hidden" value="<?= $data['id'] ?>" name="id" id="id">
                            <li class="col-6 p-2">
                                <label class="fw-light" for="firstName"><small>First Name</small></label>
                                <input type="text" value="<?= $data['firstName'] ?>" name="firstName" id="firstName" placeholder="first name" class="form-control rounded-3" autocomplete="off" required>
                            </li>
                            <li class="col-6 p-2">
                                <label class="fw-light" for="lastName"><small>Last Name</small></label>
                                <input type="text" value="<?= $data['lastName'] ?>" name="lastName" id="lastName" placeholder="last name" class="form-control rounded-3" autocomplete="off" required>
                            </li>
                            <li class="col-6 p-2">
                                <label class="fw-light" for="username"><small>Username</small></label>
                                <input type="text" value="<?= $data['username'] ?>" name="username" id="username" placeholder="username" class="form-control rounded-3" autocomplete="off" required>
                            </li>
                            <li class="col-6 p-2">
                                <label class="fw-light" for="email"><small>Email</small></label>
                                <input type="email" value="<?= $data['email'] ?>" name="email" id="email" placeholder="email" class="form-control rounded-3" autocomplete="off" required>
                            </li>
                            <li class="col-6 p-2">
                                <label class="fw-light" for="password"><small>Password</small></label>
                                <input type="password" name="password" id="password" value="<?= $data['passwordRaw'] ?>" placeholder="password" class="form-control rounded-3" autocomplete="off" required>
                            </li>
                            <li class="col-6 p-2">
                                <label class="fw-light" for="photoProfile"><small>Photo Profile</small></label>
                                <input type="file" name="photoProfile" id="photoProfile" class="form-control rounded-3" autocomplete="off" accept="image/*">
                            </li>
                            <li class="col p-2">
                                <label class="fw-light" for="role"><small>Role</small></label>
                                <select name="role" id="role" class="form-control rounded-3 my-2 w-100 text-center">
                                    <option value="" disabled>-- role --</option>
                                    <option value="Admin" <?= ($data['role'] == "Admin" ? "selected" : "") ?>>Admin</option>
                                    <option value="Author" <?= ($data['role'] == "Author" ? "selected" : "") ?>>Author</option>
                                </select>
                            </li>
                            <li class="w-100 d-flex justify-content-center align-items-center">
                                <input type="submit" value="Submit" name="submitUser" class="btn btn-dark bg-gradient shadow p-2 px-5 rounded-5">
                            </li>
                        </form>
                    </div>
                </div>
