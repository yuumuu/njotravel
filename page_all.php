...
<head>
    ...
    <title>All Pages - Dashboard | Njotravel</title>
</head>
            ...
            <div class="d-flex flex-column justify-content-start align-items-start w-100 position-relative overflow-y-auto sidebar">
                ...
                <div class="nav-side text-white w-100 pb-2">
                    ...
                    <div onclick="dropdownActive(this)" class="nav-link position-relative fs-5 py-2 bg-orange ps-3 fw-bold text-dark dropdown dropdown-active">
                        ...
                        <a href="page_all.php" class="nav-link fw-semibold"><li class="lh-1 py-2 fs-6">All Pages</li></a>
                ...
                <div class="w-100 vh-auto row px-5 py-2 my-2">
                    <div class="bg-white col p-4 mx-2 rounded-4 row shadow">
                        <div class="w-100 d-flex justify-content-between align-items-center">
                            <h3 class="text-dark fw-bold">All Pages</h3>
                            <a href="page_new.php" class="btn btn-dark fw-semibold py-2 px-3 rounded-4">Add New &plus;</a>
                        </div>
                        <table class="w-100 mt-3">
                            <thead class="w-100">
                                <tr class="w-100 row fs-5 text-center py-2 bg-white rounded-5">
                                    <th class="col-1 fw-semibold">No</th>
                                    <th class="col-4 fw-semibold text-start">Title</th>
                                    <th class="col-2 fw-semibold">Author</th>
                                    <th class="col-3 fw-semibold">Date</th>
                            ...
                            <tbody class="w-100">
                                <?php
                                    $i = 1;
                                    $query_main_page = "select * from daftar_halaman order by id limit 4";
                                    $hasil_main_page = mysqli_query($conn, $query_main_page);

                                    while($hal_utama = mysqli_fetch_assoc($hasil_main_page)) {
                                        if(strpos($hal_utama['titlePage'], " ")) {
                                            $titlePage_nospace = str_replace(" ", "_", $hal_utama['titlePage']);
                                        } else {
                                            $titlePage_nospace = $hal_utama['titlePage'];
                                        }

                                ?>
                                <tr class="w-100 row fs-6 text-center py-3">
                                    <td class="col-1 px-2 fw-normal"><?= $i++; ?></td>
                                    <td class="col-4 px-2 fw-normal text-start">
                                        <a href="<?= $titlePage_nospace ?>.php" class="text-decoration-none text-dark" target="_blank"><?= $hal_utama['titlePage'] ?></a>
                                    </td>
                                    <td class="col-2 px-2 fw-normal">Admin</td>
                                    <td class="col-3 px-2 fw-normal"><?= $hal_utama['datePage'] ?></td>
                                    <td class="col-2 px-2 fw-normal d-flex flex-row-reverse justify-content-center align-items-center">
                                        <a title="view" href="<?= $titlePage_nospace ?>.php" class="btn btn-secondary py-1 px-3 rounded-5 shadow mx-1 fst-italic">
                                            &#8505;
                                        </a>
                                        <a title="edit" href="edit_<?= $titlePage_nospace ?>.php" class="btn btn-dark py-1 px-2 rounded-5 shadow">
                                            &#9998;
                                        </a>
                                    </td>
                                </tr>
                                <?php
                                    ...
                                    $query_page = "select * from daftar_halaman where id > 9";
                                    $hasil_page = mysqli_query($conn, $query_page);

                                    while($data_page = mysqli_fetch_assoc($hasil_page)) {
                                ?>
                                <tr class="w-100 row fs-6 text-center py-3">
                                    <td class="col-1 px-2 fw-normal"><?= $i++; ?></td>
                                    <td class="col-4 px-2 fw-normal text-start">
                                        <a href="<?= $data_page['titlePage'] ?>.php" class="text-decoration-none text-dark"><?= $data_page['titlePage'] ?></a>
                                    </td>
                                    <td class="col-2 px-2 fw-normal"><?= $data_page['authorPage'] ?></td>
                                    <td class="col-3 px-2 fw-normal"><?= $data_page['datePage'] ?></td>
                                    <td class="col-2 px-2 fw-normal d-flex flex-row-reverse justify-content-center align-items-center">
                                        <form action="delete_page_process.php" method="POST">
                                            <input type="hidden" value="<?= $data_page['id'] ?>" name="id" id="id">
                                            <button title="delete" class="btn btn-danger py-1 px-3 rounded-5 shadow">
                                                &#10006;
                                            </button>
                                        </form>
                                        <a title="view" href="detail_page.php?id=<?= $data_page['id'] ?>" class="btn btn-secondary py-1 px-3 rounded-5 shadow mx-1 fst-italic">
                                            &#8505;
                                        </a>
                                        <a title="edit" href="edit_page.php?id=<?= $data_page['id'] ?>" class="btn btn-dark py-1 px-2 rounded-5 shadow">
                                            &#9998;
                                        </a>
                                    </td>
                                </tr>
